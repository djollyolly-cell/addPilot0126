...
### Блок 1.1: Кроны (heartbeat + результат)

Источник: таблица `cronHeartbeats`, `adAccounts`, `actionLogs`, `notifications`.

Принцип: проверяем НЕ ТОЛЬКО что крон запустился, но и что он СДЕЛАЛ свою работу.

#### A. Heartbeat (запуск):
- Любой крон: status == "running" > 10 мин -> STUCK (error)
- Любой крон: error != null -> CRON_ERROR (error)
- sync-metrics: finishedAt < now - 10 мин -> STALE (warning)
...
#### B. Результат (сделал ли работу):

**sync-metrics:**
- Подсчитать кабинеты с lastSyncAt < now - 10 мин (не синхронизированы)
- Сравнить с общим числом активных кабинетов
- syncedAccounts < totalActive -> SYNC_INCOMPLETE (warning): "{synced}/{total} синхронизированы"

**uz-budget-reset:**
- Для каждого uz_budget_manage правила с dailyReset=true:
  - Найти actionLogs с типом "budget_reset" за сегодня
  - Если нет записей после resetTime -> RESET_NOT_EXECUTED (error)
  - Если записей меньше чем targetCampaignIds -> PARTIAL_RESET (error): "{done}/{total} кампаний сброшены"

**daily-digest:**
- Если finishedAt сегодня: проверить notifications с type="digest" за сегодня
- Пользователи с digestEnabled=true но без записи notification -> DIGEST_NOT_SENT (warning)

**process-expired-subscriptions:**
- Пользователи с subscriptionExpiresAt < now и tier != "freemium" -> DOWNGRADE_MISSED (error)
  (дублирует блок 1.5, но здесь мы понимаем что крон НЕ обработал)

**agency-token-health:**
- Кабинеты с агентским подключением: lastError содержит "token" и обновлён за 24ч -> TOKEN_CHECK_FOUND_ISSUES (warning)
- Крон finishedAt свежий, но ни одного обновления lastError -> либо всё ок, либо крон пустой

Вердикт: "ok: все кроны работают и дают результат" / "warning: N кронов отстают или неполные" / "error: N кронов не выполнили задачу"
...
**Проверка ежедневного сброса (dailyReset):**

Для каждого uz_budget_manage правила с dailyReset=true:
1. Получить initialBudget из правила
2. Получить текущий budget_limit_day из VK API для каждой кампании
3. Найти actionLogs с типом "budget_reset" за сегодня

Ищем:
- Сейчас > resetTime + 1ч И budget > initialBudget -> RESET_FAILED (error): "Кампания '{name}': бюджет {budget}, должен быть {initialBudget} после сброса"
- Нет записи о ресете за сегодня -> RESET_NOT_LOGGED (error): крон запустился но не выполнил ресет для этого правила
- Ресет выполнен частично (3/5 кампаний сброшены) -> PARTIAL_RESET (error): возможно токен упал в середине

Зачем: ловит ситуацию Артёма Валутова — бюджеты 104-129 вместо 100 после полуночи. Без этого блока пользователь обнаруживает что бюджеты не сбросились только когда видит перерасход.
