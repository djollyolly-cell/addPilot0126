...
**Также:** Спеки описывают лимиты на количество менеджеров (1/3/10/20) и niches (3/6/all), но в коде `TIERS` нет полей `maxManagers` или `maxNiches`. Эти лимиты либо не реализованы, либо хардкожены где-то ещё.

**Действие:** Обновить все Plan-документы (1-6) и Spec-документы с актуальными цифрами из кода, либо пометить Plans как superseded.
...
Phase 1 — Security + Manager basics (блокирует запуск)
  #1  clientReport.buildReport — access control          [P0, 30 min]
  #2  adAccounts.list — getAccessibleAccountIds          [P1, 1h]
  #3  adAccounts.connect — set orgId                     [P1, 30 min]
  #4  rules.list — org-aware query                       [P1, 1h]
  #5  rules update/toggle/remove — checkOrgWritable      [P1, 1h]
  #6  tier limits — org tier for org-users               [P1, 30 min]

Phase 2 — Data integrity (блокирует корректную работу правил)
  #7  conflict validation — orgId scope                  [P2, 1h]
  #8  rule engine — load rules by account/org            [P2, 2h]
  #9  rule engine — grace check before execution         [P2, 30 min]
  #10 deleteUser — agencyRequests cascade                [P2, 15 min]
  #11 adAccounts.disconnect — owner access               [P2, 30 min]

Phase 3 — Limits + docs
  #12 update plan docs with real pricing                 [P3, 1h]
  #14 manager limits + niche limits enforce              [P3, 2h]
